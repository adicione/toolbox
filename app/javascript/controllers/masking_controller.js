...
export default class extends Controller {
  ...
  disconnect() {
    ...
    this.fields_to_mask.forEach(field => {
      field.removeEventListener("input", event => { this.maskInput(field) })
  ...
  setup() {
    ...
    this.fields_to_mask = Object.keys(masks).flatMap(mask =>
      Array.from(this.element.querySelectorAll(`.${ mask }`)).map(field => {
        // Sets the input type for later use.
        field.addEventListener("input", event => {
          field.mask = masks[mask].call(this, field.value)
          field.inputType = event.inputType

          this.maskInput(field)
        })

        // Applies immediately if there's an initial value.
        if (field.value) { this.maskInput(field) }
  ...
  maskInput(field) {
    ...
    field.value = field.mask.result

    if (document.activeElement !== field) return // Adjusts position based on inputType and maskPosition if focused.

    const isInsertion = !["deleteContentBackward", "deleteContentForward"].includes(field.inputType || "insertText")

    while (field.mask.maskPositions.includes(position)) { if (isInsertion) { position++ } else { position-- } }
